...
//***************************
//    deconvolved flipped integral analysis
...
int main(int argc, char** argv) {
   ...
   TFile f_output("flippingdecon_output.root","RECREATE");
  ...
  TCanvas c7("notflippedint_decon","c7",900,600);
  TH2F hIntFlipped("hIntFlipped", "Deconvolved ROI Integral with Flipped Bit; Channel; ROI integral (ADC)", 8256, 0, 8256, 8000, 0, 8000);
  TH2F hIntNot("hIntNot", "Deconvolved ROI Integral No Flipped Bit; Channel; ROI integral (ADC)", 8256, 0, 8256, 8000, 0, 8000);
  TCanvas c8("flippedint_decon","c8",900,600);
  ...
  for (gallery::Event ev(filenames) ; !ev.atEnd(); ev.next()) {
	...
	const float medianCut = 15; // Maximum absolute difference between a sample and the median to be considered as baseline // Add to FHiCL file?      
	  ...
	  TH1D horig1("roi_original1", "roi_original;Tick;ADC", endTick - firstTick, firstTick, endTick);
          for (size_t iTick = ROI.begin_index(); iTick < ROI.end_index(); iTick++ ){
            horig1.Fill((int)iTick,ROI[iTick]);}
          TCanvas c2(Form("c1_%d_%d_na",event,channel),"c1",900,600);
